...
void CypressGotoPosition::Initialize()
{
	...
	if (DriverStation::GetInstance()->GetEnhancedIO().GetButton(INPUT_SHOOTER_ANGLE_ADJUSTMENT_MANUAL) == false)
	{
		AddParallel(new SetShooterAngle(m_angle));
	}
	
	// Check to see if automated shooter speed
	if (DriverStation::GetInstance()->GetEnhancedIO().GetButton(INPUT_SHOOTER_SPEED_ADJUSTMENT_MANUAL) == false)
	{
