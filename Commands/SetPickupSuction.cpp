...
SetPickupSuction::SetPickupSuction() 
{
	...
	suckSetInConstructor = false;
	SmartDashboard::PutBoolean("Pickup Suction", Robot::pickup->suction->Get());
}
SetPickupSuction::SetPickupSuction(bool suck_parameter)
{
	suck = suck_parameter;
	suckSetInConstructor = true;
}
// Called just before this Command runs the first time
void SetPickupSuction::Initialize() 
{
	// Check if the suction is already set
	if(suckSetInConstructor == false)
	{
		suck = 	SmartDashboard::GetBoolean("Pickup Suction");
	}
	
	Robot::pickup->suction->Set(suck);
}
