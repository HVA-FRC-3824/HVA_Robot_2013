// RobotBuilder Version: 0.0.2
//
// This file was generated by RobotBuilder. It contains sections of
// code that are automatically generated and assigned by robotbuilder.
// These sections will be updated in the future when you export to
// C++ from RobotBuilder. Do not put any code or make any change in
// the blocks indicating autogenerated code or it will be lost on an
// update. Deleting the comments indicating the section will prevent
// it from being updated in th future.
#include "Robot.h"
#include "Commands/AutonomousDoNothing.h"
#include "Commands/AutonomousDriveForwardShoot.h"
#include "Commands/AutonomousRPMDriveForwardShoot.h"
#include "Commands/AutonomousRPMShootfromCenter.h"
#include "Commands/AutonomousRPMShootfromCenterUnderGoal.h"
#include "Commands/AutonomousRPMShootfromLeft.h"
#include "Commands/AutonomousRPMShootfromRight.h"
#include "Commands/AutonomousRPMShootMiddleGoalHard.h"
#include "Commands/AutonomousRPMShootMiddleGoalHardtoCenterline.h"
#include "Commands/AutonomousRPMShootMiddleGoalSoft.h"
#include "Commands/AutonomousRPMShootTopGoalfromSide.h"
#include "Commands/AutonomousShootFromCenter.h"
#include "Commands/AutonomousShootfromCenterUnderGoal.h"
#include "Commands/AutonomousShootFromLeft.h"
#include "Commands/AutonomousShootFromRight.h"
#include "Commands/AutonomousShootMiddleGoalHard.h"
#include "Commands/AutonomousShootMiddleGoalHardtoCenterline.h"
#include "Commands/AutonomousShootMiddleGoalSoft.h"
#include "Commands/AutonomousShootTopGoalfromSide.h"
#include "Commands/Teleoperated.h"
// BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=INITIALIZATION
Drivetrain* Robot::drivetrain = 0;
ShooterWheel* Robot::shooterWheel = 0;
ShooterPusher* Robot::shooterPusher = 0;
ShooterAngleAdjust* Robot::shooterAngleAdjust = 0;
Pump* Robot::pump = 0;
Pickup* Robot::pickup = 0;
OI* Robot::oi = 0;
    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=INITIALIZATION
void Robot::RobotInit()
{
   // Must be called before constructing OI
   // PID values for the turn angle
   SmartDashboard::PutNumber("Chassis Turn P Term", 0.04);   // Tuned for Robot 1!
   SmartDashboard::PutNumber("Chassis Turn I Term", 0.004);
   SmartDashboard::PutNumber("Chassis Turn D Term", 0.04);
   
   SmartDashboard::PutNumber("Chassis Turn Angle Degrees", 10);
   
   // PID value for aimer
   SmartDashboard::PutNumber("Shooter Aim P: ", 0.01);
   SmartDashboard::PutNumber("Shooter Aim I: ", 0.0);
   SmartDashboard::PutNumber("Shooter Aim D: ", 0.0);
   
   // PID values for the shooter speed
   SmartDashboard::PutNumber("Shooter Speed P Term", 0.02);
   SmartDashboard::PutNumber("Shooter Speed I Term", 0.001);
   SmartDashboard::PutNumber("Shooter Speed D Term", 0.0);
   
   // PID values for the shooter angle
   SmartDashboard::PutNumber("Shooter Angle P Term", 0.03);
   SmartDashboard::PutNumber("Shooter Angle I Term", 0.0);
   SmartDashboard::PutNumber("Shooter Angle D Term", 0.005); 
   
   // PID value for pickup
   SmartDashboard::PutNumber("Pickup P Term", 0.02);
   SmartDashboard::PutNumber("Pickup I Term", 0.001);
   SmartDashboard::PutNumber("Pickup D Term", 0.009); 
   
   // Defaults to Search for target
   SmartDashboard::PutString("Target Type", "target");
   RobotMap::init();
   
   // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTRUCTORS
	drivetrain = new Drivetrain();
	shooterWheel = new ShooterWheel();
	shooterPusher = new ShooterPusher();
	shooterAngleAdjust = new ShooterAngleAdjust();
	pump = new Pump();
	pickup = new Pickup();
    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTRUCTORS
   // This MUST be here. If the OI creates Commands (which it very likely
   // will), constructing it during the construction of CommandBase (from
   // which commands extend), subsystems are not guaranteed to be
   // yet. Thus, their requires() statements may grab null pointers. Bad
   // news. Don't move it.
   oi = new OI();
   lw = LiveWindow::GetInstance();
   // instantiate the command used for the autonomous period
   // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=AUTONOMOUS
	autonomousCommand = new AutonomousCommand();
    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=AUTONOMOUS
   
	// These must be after subsystem construction.
   chooser = new SendableChooser();
   chooser->AddDefault("Do Nothing", new AutonomousDoNothing());
//   chooser->AddObject("Drive forward and shoot - voltage", new AutonomousDriveForwardShoot());
   chooser->AddObject("Drive forward and shoot - RPM", new AutonomousRPMDriveForwardShoot());
   chooser->AddObject("Shoot from Center - RPM", new AutonomousRPMShootfromCenter());
   chooser->AddObject("Shoot from Center under Goal - RPM", new AutonomousRPMShootfromCenterUnderGoal());
//   chooser->AddObject("Shoot from Left - RPM", new AutonomousRPMShootfromLeft());
//   chooser->AddObject("Shoot from Right - RPM", new AutonomousRPMShootfromRight());
   chooser->AddObject("Shoot Middle Goal Hard - RPM", new AutonomousRPMShootMiddleGoalHard());
//   chooser->AddObject("Shoot Middle Goal Hard to Centerline - RPM", new AutonomousRPMShootMiddleGoalHardtoCenterline());
   chooser->AddObject("Shoot Middle Goal Soft - RPM", new AutonomousRPMShootMiddleGoalSoft());
   chooser->AddObject("Shoot Top Goal from Side - RPM", new AutonomousRPMShootTopGoalfromSide());
//   chooser->AddObject("Shoot from Center - voltage", new AutonomousShootfromCenter());
//   chooser->AddObject("Shoot from Center under Goal- voltage", new AutonomousShootfromCenterUnderGoal());
//   chooser->AddObject("Shoot from Left - voltage", new AutonomousShootfromLeft());
//   chooser->AddObject("Shoot from Right - voltage", new AutonomousShootfromRight());
//   chooser->AddObject("Shoot Middle Goal Hard - voltage", new AutonomousShootMiddleGoalHard());
//   chooser->AddObject("Shoot Middle Goal Hard to Centerline - voltage", new AutonomousShootMiddleGoalHardtoCenterline());
//   chooser->AddObject("Shoot Middle Goal Soft - voltage", new AutonomousShootMiddleGoalSoft());
//   chooser->AddObject("Shoot Top Goal from Side - voltage", new AutonomousShootTopGoalfromSide());
   
   // show the autonomous modes
   SmartDashboard::PutData("Autonomous modes:", chooser);
   
   // Instanciate the Teleop Command
   teleop = new Teleoperated();
}
void Robot::AutonomousInit()
{
   // get the autonomous command from the smartdashboard chooser
   autonomousCommand = (Command *) chooser->GetSelected();
   
   // ensure the autonomous command exists and run
   if (autonomousCommand != NULL)
      autonomousCommand->Start();
}
void Robot::AutonomousPeriodic()
{
   Scheduler::GetInstance()->Run();
}
void Robot::TeleopInit()
{
   // This makes sure that the autonomous stops running when
   // teleop starts running. If you want the autonomous to 
   // continue until interrupted by another command, remove
   // this line or comment it out.
   autonomousCommand->Cancel();
   
   // Start the Teleop Command
   teleop->Start();
}
void Robot::TeleopPeriodic()
{
   if (autonomousCommand != NULL)
      Scheduler::GetInstance()->Run(); 
}
void Robot::TestPeriodic()
{
   lw->Run();
}
START_ROBOT_CLASS(Robot);
